...
function getStaleSessionTTL(cwd) {
  try {
    ...
    const configPath = path.join(cwd, '.hefaisto-core', 'core-config.yaml');
...
function resolveHookRuntime(input) {
  ...
  try {
    const { loadSession, cleanStaleSessions } = require(
      path.join(cwd, '.hefaisto-core', 'core', 'synapse', 'session', 'session-manager.js'),
    );
    const { SynapseEngine } = require(
      path.join(cwd, '.hefaisto-core', 'core', 'synapse', 'engine.js'),
    );
