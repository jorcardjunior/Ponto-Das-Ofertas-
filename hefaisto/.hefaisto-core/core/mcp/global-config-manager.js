 ...
 *
 * Manages the global MCP configuration at ~/.hefaisto/mcp/
...
/**
 * Check if global Hefaisto directory exists
 * @returns {boolean}
 */
function globalDirExists() {
  return fs.existsSync(getGlobalHefaistoDir());
}
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "hefaisto mcp setup" first.' };
  }
