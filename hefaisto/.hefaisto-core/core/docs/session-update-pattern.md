...
The session update pattern enables intelligent greeting adaptation by tracking command execution history and agent transitions. This allows Hefaisto to provide contextual greetings that reflect workflow continuity.
...
```javascript
const { updateSessionAfterCommand } = require('./.hefaisto-core/scripts/command-execution-hook');
...
- [ ] Add agent transition tracking to `/Hefaisto/agents/*` commands
...
- **Path:** `.hefaisto-core/.session/current-session.json`
...
```bash
# Clear session
rm .hefaisto-core/.session/current-session.json

# Test new session greeting
node .hefaisto-core/development/scripts/test-greeting-system.js

# Simulate command
node -e "require('./.hefaisto-core/scripts/command-execution-hook').updateSessionAfterCommand('dev', 'develop-yolo')"
...
```bash
ls -la .hefaisto-core/.session/
cat .hefaisto-core/.session/current-session.json
```
...
- [Story 6.1.4 Implementation](../../stories/hefaisto migration/story-6.1.4.md)
