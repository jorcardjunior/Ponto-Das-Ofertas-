# Synkra Hefaisto Meta-Agent Troubleshooting Guide

## Overview

This guide helps diagnose and resolve common issues when using the Synkra Hefaisto meta-agent for component creation and management.
...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /hefaisto-core/agents/data-analyst.md
```
...
2. Check existing components:
   ```bash
   ls hefaisto-core/agents/
   ```

3. If overwriting is intended:
   ```bash
   # Remove existing component first
   rm hefaisto-core/agents/data-analyst.md
   *create-agent
   ```
...
3. Check working directory:
   ```bash
   pwd
   # Should be in hefaisto-core root
   ```
...
1. Create session directory:
   ```bash
   mkdir -p hefaisto-core/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 hefaisto-core/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls hefaisto-core/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la hefaisto-core/agents/file1.md
   
   # Remove manually if needed
   rm hefaisto-core/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls hefaisto-core/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat hefaisto-core/logs/transactions/latest.json
   ```
...
# Specific modules
DEBUG=hefaisto:template,hefaisto:elicitation *create-agent
...
```bash
# Application logs
tail -f hefaisto-core/logs/hefaisto-developer.log

# Transaction logs
ls -la hefaisto-core/logs/transactions/

# Error logs
grep ERROR hefaisto-core/logs/*.log
```
...
- Template syntax: `hefaisto-core/docs/template-syntax.md`
- Creation guide: `hefaisto-core/docs/component-creation-guide.md`
- API reference: `hefaisto-core/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf hefaisto-backup.tar.gz hefaisto-core/
   ```

2. **Reset to clean state**:
   ```bash
   git checkout -- hefaisto-core/
   npm run setup
   ```
