 ...
 * Scans the .hefaisto-core directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.hefaistoCoreDir = path.join(this.basePath, '.hefaisto-core');
    this.manifestDir = path.join(this.hefaistoCoreDir, 'manifests');
    this.version = '2.1.0';
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.hefaistoCoreDir, 'development', 'agents');
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: 'active',
              file_path: `.hefaisto-core/development/agents/${file}`,
  ...
  async generateWorkersManifest() {
    const registryPath = path.join(this.hefaistoCoreDir, 'core', 'registry', 'service-registry.json');
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.hefaistoCoreDir, 'development', 'tasks');
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.hefaisto-core/development/tasks/${file}`,
            status: 'active',
          });
