...
const MONITOR_SERVER_URL = process.env.Hefaisto_MONITOR_URL || 'http://localhost:4001/events';
...
class DashboardEmitter {
  ...
  constructor() {
    ...
    this.fallbackPath = path.join(this.projectRoot, '.hefaisto', 'dashboard', 'events.jsonl');
  ...
  async emit(type, data = {}) {
    ...
    const event = {
      ...
      hefaisto_agent: this.currentAgent || undefined,
      hefaisto_story_id: this.currentStoryId || undefined,
      data,
    };
      ...
      const response = await fetch(MONITOR_SERVER_URL, {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            hefaisto_agent: event.hefaisto_agent,
            hefaisto_story_id: event.hefaisto_story_id,
          },
        }),
