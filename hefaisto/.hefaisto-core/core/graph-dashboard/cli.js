...
/**
 * Write HTML graph to .hefaisto/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), '.hefaisto');
 ...
 * Writes to .hefaisto/graph.dot, .hefaisto/graph.mmd, or .hefaisto/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), '.hefaisto-core/data/entity-registry.yaml');
...
function handleHelp() {
  const usage = `
Usage: hefaisto graph [command] [options]
...
Examples:
  hefaisto graph --deps                        Show dependency tree
  hefaisto graph --deps --format=json          Output as JSON
  hefaisto graph --deps --format=html          Interactive HTML graph (opens browser)
  hefaisto graph --deps --watch                Live DOT file for VS Code preview
  hefaisto graph --deps --watch --format=html  Live HTML with auto-refresh
  hefaisto graph --deps --watch --format=mermaid  Live Mermaid file
  hefaisto graph --deps --watch --interval=10  Refresh every 10 seconds
  hefaisto graph --stats                       Show entity stats and cache metrics
`.trim();
...
async function handleSummary(args) {
  ...
  sections.push('Hefaisto Graph Dashboard');
