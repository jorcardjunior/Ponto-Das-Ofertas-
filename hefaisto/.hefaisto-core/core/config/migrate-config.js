 ...
 *   - .hefaisto-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.hefaisto/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.hefaisto/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.hefaisto-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.hefaisto-core', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.hefaisto-core', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.hefaisto-core', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.hefaisto-core', 'core-config.backup.yaml');
...
/**
 * Ensure the ~/.hefaisto/ directory exists.
 *
 * @returns {string} Path to ~/.hefaisto/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.hefaisto');
...
/**
 * Write user config to ~/.hefaisto/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.hefaisto', 'user-config.yaml');
...
/**
 * Write project config to .hefaisto-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.hefaisto-core', 'project-config.yaml');
