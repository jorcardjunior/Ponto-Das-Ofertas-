 ...
 *
 * Provides async API for spawning Hefaisto agents in separate terminals
...
async function createContextFile(context, outputDir = os.tmpdir()) {
  ...
  const contextPath = path.join(outputDir, `hefaisto-context-${Date.now()}.json`);
...
async function spawnInline(agent, task, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn('bash', [scriptPath, ...args], {
      env: {
        ...process.env,
        Hefaisto_DEBUG: opts.debug ? 'true' : 'false',
        Hefaisto_OUTPUT_DIR: opts.outputDir,
        Hefaisto_INLINE_MODE: 'true', // Signal to pm.sh that we're running inline
      },
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const hefaistoFiles = files.filter(
      (f) => f.startsWith('hefaisto-output-') || f.startsWith('hefaisto-lock-') || f.startsWith('hefaisto-context-'),
    );

    for (const file of hefaistoFiles) {
...
function formatCompatibilityReport(report) {
  const lines = [
    '═══════════════════════════════════════════════════════════════',
    '                 Hefaisto Terminal Spawner Compatibility Report     ',
    '═══════════════════════════════════════════════════════════════',
    '',
