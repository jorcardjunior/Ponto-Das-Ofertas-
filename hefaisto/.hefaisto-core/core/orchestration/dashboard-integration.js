 ...
 *
 * Features:
 * - AC1: Updates `.hefaisto/dashboard/status.json`
...
class DashboardIntegration extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // Paths
    this.dashboardDir = path.join(this.projectRoot, '.hefaisto', 'dashboard');
    this.statusPath = path.join(this.dashboardDir, 'status.json');
    this.logsDir = path.join(this.projectRoot, '.hefaisto', 'logs');
