 ...
 *
 * Verifies that Hefaisto framework configuration files are present.
 *
 * @module hefaisto-core/health-check/checks/project/framework-config
 * @version 1.0.0
...
const REQUIRED_CONFIGS = [
  { path: '.hefaisto-core', type: 'directory', description: 'Hefaisto core framework' },
...
const RECOMMENDED_CONFIGS = [
  { path: '.hefaisto', type: 'directory', description: 'Hefaisto local configuration' },
...
class FrameworkConfigCheck extends BaseCheck {
  constructor() {
    super({
      ...
      description: 'Verifies Hefaisto framework configuration is present',
      domain: CheckDomain.PROJECT,
      severity: CheckSeverity.HIGH,
      timeout: 3000,
      cacheable: true,
      healingTier: 0, // Cannot auto-fix framework setup
      tags: ['hefaisto', 'config', 'framework'],
    });
  }
  ...
  async execute(context) {
    ...
    if (missingRequired.length > 0) {
      ...
      return this.fail(`Missing required framework configuration: ${missing}`, {
        recommendation: 'Run Hefaisto setup or manually create missing directories',
        details: {
    ...
    if (missingRecommended.length > 0) {
      ...
      return this.warning(`Missing recommended configuration: ${missing}`, {
        recommendation: 'Consider adding recommended configuration for full Hefaisto functionality',
        details: {
