 ...
 *
 * @module hefaisto-core/health-check/checks/services/gemini-cli
 * @version 1.0.0
...
class GeminiCliCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Check for rules.md (Hefaisto rules)
      try {
      ...
      // Check for Hefaisto agents
      try {
        const agentsDir = path.join(geminiDir, 'rules', 'Hefaisto', 'agents');
        const agentFiles = await fs.readdir(agentsDir);
        details.hefaistoAgents = agentFiles.filter((f) => f.endsWith('.md')).length;
      } catch {
        details.hefaistoAgents = 0;
      }
    } catch {
      // No project config
    }
    ...
    if (details.projectConfig && details.hefaistoAgents === 0) {
      warnings.push('No Hefaisto agents installed for Gemini CLI');
    }
    ...
    if (issues.length > 0) {
      return this.warning(`Gemini CLI needs attention: ${issues.join(', ')}`, {
        recommendation: issues.includes('Not authenticated')
          ...
          : 'Run `npx hefaisto-core install` and select Gemini CLI',
    ...
    if (warnings.length > 0) {
      return this.warning(`Gemini CLI configuration incomplete: ${warnings.join(', ')}`, {
        recommendation: 'Run `npx hefaisto-core install` and select Gemini CLI',
    ...
    if (details.hefaistoAgents > 0) parts.push(`${details.hefaistoAgents} Hefaisto agents`);
