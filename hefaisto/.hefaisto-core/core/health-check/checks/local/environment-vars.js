 ...
 *
 * @module hefaisto-core/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * Hefaisto-specific environment variables
 */
const Hefaisto_VARS = ['Hefaisto_DEBUG', 'Hefaisto_LOG_LEVEL'];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check Hefaisto-specific vars (info only)
    for (const varName of Hefaisto_VARS) {
      if (process.env[varName]) {
        hefaistoVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
