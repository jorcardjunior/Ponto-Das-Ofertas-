...
class FocusAreaRecommender {
  ...
  analyzeChangedFiles(changedFiles = []) {
    ...
    const categoryPatterns = [
      ...
      },
      {
        ...
        category: 'hefaisto-core',
        highlight: 'Hefaisto framework changes',
        risk: 'high',
      },
      {
  ...
  determineSecondaryAreas(fileAnalysis, _layer2Result) {
    ...
    const secondaryCategories = ['ux', 'configuration', 'performance', 'hefaisto-core'];
  ...
  getReviewQuestions(category) {
    const questionBank = {
      ...
      'hefaisto-core': [
        'Does this follow Hefaisto framework patterns?',
