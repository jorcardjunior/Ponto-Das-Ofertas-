 ...
 *
 * Subcommands:
 *   hefaisto config show [--level <n>] [--app <name>] [--debug]
 *   hefaisto config diff --levels <a>,<b>
 *   hefaisto config migrate [--dry-run] [--force]
 *   hefaisto config validate [--level <n>]
...
// Resolve core config modules (relative from .hefaisto-core/cli/commands/config/)
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log('This may be expected due to key normalization. Run `hefaisto config diff` to inspect.');
    }
...
function splitL1(config) {
  ...
  l1.metadata = { name: 'Synkra Hefaisto', framework_version: '3.12.0' };
...
function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, '.hefaisto-core', 'local-config.yaml.template');
  const targetPath = path.join(root, '.hefaisto-core', 'local-config.yaml');

  if (!fs.existsSync(templatePath)) {
    console.error('Template not found: .hefaisto-core/local-config.yaml.template');
    process.exit(1);
  }
  ...
  console.log('Created: .hefaisto-core/local-config.yaml (from template)');
...
/**
 * Create the `hefaisto config` command with all subcommands.
 * @returns {Command}
 */
