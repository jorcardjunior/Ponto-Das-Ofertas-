 ...
 * MCP Setup Command
 *
 * Creates the global ~/.hefaisto/mcp/ directory structure.
 *
...
function createSetupCommand() {
  ...
  setup
    .description('Create global MCP configuration structure at ~/.hefaisto/')
...
async function executeSetup(options) {
  ...
  console.log(`  ${getGlobalHefaistoDir()}/`);
  console.log('  ├── mcp/');
  ...
  console.log('\nNext steps:');
  console.log('  1. Run "hefaisto mcp link" in your project to use global config');
  console.log('  2. Run "hefaisto mcp add <server>" to add more servers');
  console.log('  3. Run "hefaisto mcp status" to check configuration');
