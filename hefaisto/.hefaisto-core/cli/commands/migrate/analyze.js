...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(hefaistoCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .hefaisto-core directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(hefaistoCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(hefaistoCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect Hefaisto version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .hefaisto-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(hefaistoCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(hefaistoCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(hefaistoCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(hefaistoCoreDir, moduleName);
