...
function transformImportPath(importPath, currentFileModule, currentFilePath, transformMap) {
  ...
  if (importPath.startsWith('../') || importPath.startsWith('./')) {
    ...
    // 1. If the path goes up to .hefaisto-core root and then into a folder, update it
    ...
    // If going up enough to reach .hefaisto-core root
    if (restParts.length > 0) {
      ...
      for (const [moduleName, config] of Object.entries(MODULE_MAPPING)) {
        if (config.directories.includes(targetDir)) {
          ...
          // Current structure after migration: .hefaisto-core/{module}/...
          // Need to go to: .hefaisto-core/{targetModule}/...
  ...
  try {
    ...
    for (const imp of imports) {
      const relativePath = path.relative(
        path.dirname(filePath).split('.hefaisto-core')[1]?.slice(1) || '',
        '',
      );
 ...
 * @param {string} hefaistoCoreDir - Path to .hefaisto-core
 * @param {Object} plan - Migration plan
 * @param {Object} options - Options
 * @returns {Promise<Object>} Update results
 */
async function updateAllImports(hefaistoCoreDir, plan, options = {}) {
  ...
  for (const moduleName of modules) {
    const moduleDir = path.join(hefaistoCoreDir, moduleName);
    ...
    for (const file of files) {
      ...
      if (verbose && updateResult.updated > 0) {
        ...
        onProgress({
          phase: 'file',
          message: `  → ${path.relative(hefaistoCoreDir, file)} (${updateResult.updated} imports)`,
        });
      }
    }
  }
 ...
 */
async function verifyImports(hefaistoCoreDir) {
  const result = {
    valid: true,
    totalImports: 0,
    brokenImports: [],
        ...
        if (imp.importPath.startsWith('.')) {
          ...
          if (!found) {
            result.brokenImports.push({
              file: path.relative(hefaistoCoreDir, file),
