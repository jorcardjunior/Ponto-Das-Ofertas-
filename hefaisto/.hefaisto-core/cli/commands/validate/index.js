 ...
 *
 * CLI command for validating Hefaisto-Core installation integrity.
 ...
 *
 * Usage:
 *   hefaisto validate                    # Validate current installation
 *   hefaisto validate --repair           # Repair missing/corrupted files
 *   hefaisto validate --repair --dry-run # Preview repairs without applying
 *   hefaisto validate --detailed         # Show detailed file list
 *   hefaisto validate --json             # Output as JSON
 */
...
function createValidateCommand() {
  ...
  validate
    .description('Validate Hefaisto-Core installation integrity')
...
async function runValidation(options) {
  ...
  const hefaistoCoreDir = path.join(projectRoot, '.hefaisto-core');

  // Check if Hefaisto-Core is installed
  if (!fs.existsSync(hefaistoCoreDir)) {
    if (options.json) {
      console.log(
        JSON.stringify(
          {
            status: 'failed',
            error: 'Hefaisto-Core not found in current directory',
            // SECURITY: Sanitize path - only show relative indicator
            location: '.hefaisto-core',
          },
          null,
          2
        )
      ...
      console.error(chalk.dim(`Expected at: ${hefaistoCoreDir}`));
      console.error(chalk.dim('\nRun `npx hefaisto-core install` to install Hefaisto-Core'));
    }
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, '.hefaisto-core', 'install-manifest.yaml');
  ...
  if (!sourceDir && options.repair) {
    ...
    const possibleSources = [
      ...
      path.join(projectRoot, 'node_modules/hefaisto-core'),
      path.join(projectRoot, 'node_modules/hefaisto-core'),
    ];

    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, '.hefaisto-core', 'install-manifest.yaml'))) {
        sourceDir = src;
        break;
      }
    }
