...
/**
 * Git Post-Commit Hook - Hefaisto ProjectStatusLoader Cache Invalidation
 *
 ...
 *     node .hefaisto-core/infrastructure/scripts/git-hooks/post-commit.js
 */
 ...
 * looking for .hefaisto-core directory.
 *
...
function findProjectRoot() {
  ...
  // Walk up looking for .hefaisto-core
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, '.hefaisto-core'))) {
      return dir;
    }
...
/**
 * Clear all project-status cache files in the .hefaisto directory.
 * Handles both standard cache and worktree-specific cache files.
 */
function clearProjectStatusCache() {
  const projectRoot = findProjectRoot();
  const hefaistoDir = path.join(projectRoot, '.hefaisto');

  if (!fs.existsSync(hefaistoDir)) {
    return; // No .hefaisto directory - nothing to clear
  }

  try {
    const files = fs.readdirSync(hefaistoDir);

    for (const file of files) {
      // Match project-status.yaml and project-status-{hash}.yaml
      if (file.startsWith('project-status') && file.endsWith('.yaml')) {
        const filePath = path.join(hefaistoDir, file);
        try {
