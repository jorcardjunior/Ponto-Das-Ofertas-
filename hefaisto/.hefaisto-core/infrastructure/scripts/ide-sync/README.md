...
Automatically synchronizes Hefaisto agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.hefaisto-core/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/Hefaisto/agents/` (Claude Code)
- `.codex/agents/` (Codex CLI support files)
- `.gemini/rules/Hefaisto/agents/` (Gemini CLI)
...
npm run sync:ide
# or
node .hefaisto-core/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .hefaisto-core/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .hefaisto-core/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.hefaisto-core/core-config.yaml`:

```yaml
ideSync:
  enabled: true
  source: .hefaisto-core/development/agents
  targets:
    claude-code:
      enabled: true
      path: .claude/commands/Hefaisto/agents
    ...
    gemini:
      enabled: true
      path: .gemini/rules/Hefaisto/agents
      format: full-markdown-yaml
    github-copilot:
      enabled: true
      path: .github/agents
    ...
    # ... other IDEs
  redirects:
    hefaisto-developer: hefaisto-master
...
```markdown
# Agent Redirect: hefaisto-developer -> hefaisto-master

This agent has been renamed. Use `hefaisto-master` instead.
```

## File Structure

```
.hefaisto-core/infrastructure/scripts/ide-sync/
