...
function getDefaultOptions() {
  ...
  return {
    ...
    agentsDir: path.join(projectRoot, '.gemini', 'rules', 'Hefaisto', 'agents'),
    commandsDir: path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: path.join(projectRoot, 'packages', 'gemini-hefaisto-extension'),
    sourceAgentsDir: path.join(projectRoot, '.hefaisto-core', 'development', 'agents'),
    quiet: false,
    json: false,
  };
}
...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    agentsDir: options.agentsDir || path.join(projectRoot, '.gemini', 'rules', 'Hefaisto', 'agents'),
    commandsDir: options.commandsDir || path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: options.extensionDir || path.join(projectRoot, 'packages', 'gemini-hefaisto-extension'),
    sourceAgentsDir: options.sourceAgentsDir || path.join(projectRoot, '.hefaisto-core', 'development', 'agents'),
  };
  const errors = [];
  const warnings = [];
  ...
  if (!commandFiles.includes('hefaisto-menu.toml')) {
    errors.push(`Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, 'hefaisto-menu.toml'))}`);
  }
  ...
  const requiredExtensionFiles = [
    'extension.json',
    'README.md',
    path.join('commands', 'hefaisto-status.js'),
    path.join('commands', 'hefaisto-agents.js'),
    path.join('commands', 'hefaisto-validate.js'),
