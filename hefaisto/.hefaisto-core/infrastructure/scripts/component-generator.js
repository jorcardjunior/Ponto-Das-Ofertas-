/**
 * Component Generator for Synkra Hefaisto
...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'hefaisto-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'hefaisto-core', 'agents'),
      task: path.join(this.rootPath, 'hefaisto-core', 'tasks'),
      workflow: path.join(this.rootPath, 'hefaisto-core', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using Synkra Hefaisto template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'Synkra Hefaisto';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'hefaisto-core', 'tasks', `${taskId}.md`);
