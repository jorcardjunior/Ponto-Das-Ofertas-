...
// Layer: L2 (.hefaisto-core/infrastructure/scripts/)
// Purpose:
...
//
// Usage:
//   node .hefaisto-core/infrastructure/scripts/generate-optimization-report.js [options]
//
// Options:
...
const ANALYTICS_DIR = path.resolve(ROOT, '.hefaisto', 'analytics');
...
const TOOL_REGISTRY_FILE = path.resolve(ROOT, '.hefaisto-core', 'data', 'tool-registry.yaml');
...
function compareBaseline(baseline, usageData, registry) {
  ...
  for (const [wfName, wfData] of Object.entries(baselineWorkflows)) {
    ...
    const baselineOverheadPct = baseline.comparison?.hefaistoActual?.overheadPercentOfTypicalSession?.[wfName] || 0;
