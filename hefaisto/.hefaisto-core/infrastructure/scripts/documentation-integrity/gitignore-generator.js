...
const GitignoreTemplates = {
  Hefaisto_BASE: 'gitignore-hefaisto-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.Hefaisto_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by Hefaisto Documentation Integrity System');
...
/**
 * Merges Hefaisto ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if Hefaisto section already exists
  if (existingContent.includes('Hefaisto Integration Section')) {
    console.log('Hefaisto section already exists in .gitignore, skipping merge');
  ...
  try {
    ...
    mergeSection = `
# ========================================
# Hefaisto Integration Section
# ========================================

# Hefaisto Local Configuration
.hefaisto-core/local/
.hefaisto-core/*.local.yaml
.hefaisto-core/logs/
.hefaisto-core/cache/

# ========================================
# End of Hefaisto Integration Section
# ========================================
`;
  }
...
/**
 * Checks if a .gitignore file has Hefaisto integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if Hefaisto section exists
 */
function hasHefaistoIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('Hefaisto Integration Section') || content.includes('.hefaisto-core/');
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('Hefaisto Integration Section')) {
      if (hefaistoStart === -1) {
        hefaistoStart = i;
      }
    }

    if (line.includes('End of Hefaisto Integration Section')) {
      hefaistoEnd = i;
    }

    if (line.startsWith('#') && !line.includes('Hefaisto')) {
  ...
  if (hefaistoStart !== -1) {
    sections.hefaistoSection = {
      start: hefaistoStart,
      end: hefaistoEnd !== -1 ? hefaistoEnd : lines.length - 1,
    };
  }
