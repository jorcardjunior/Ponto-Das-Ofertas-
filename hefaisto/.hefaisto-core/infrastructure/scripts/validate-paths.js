...
function getDefaultOptions() {
  ...
  return {
    ...
    requiredFiles: [
      ...
      path.join(projectRoot, '.hefaisto-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
    ],
    quiet: false,
    json: false,
  };
...
function listSkillFiles(skillsDir) {
  ...
  return fs.readdirSync(skillsDir, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && entry.name.startsWith('hefaisto-'))
...
function validateSkillPathConventions(content, filePath) {
  const errors = [];
  if (!content.includes('.hefaisto-core/development/agents/')) {
    errors.push(`${filePath} missing canonical source path ".hefaisto-core/development/agents/"`);
  }
  if (!content.includes('.hefaisto-core/development/scripts/generate-greeting.js')) {
