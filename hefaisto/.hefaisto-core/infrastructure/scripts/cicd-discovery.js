 ...
 * integration points for Hefaisto workflows.
 */
...
/**
 * IntegrationSuggester - Suggests Hefaisto integration points
 */
class IntegrationSuggester {
  ...
  suggest(analysis, provider) {
    ...
    // Add Hefaisto build step
    suggestions.push({
      type: 'add_step',
      priority: 'high',
      title: 'Add Hefaisto Build Orchestration',
      description: 'Integrate Hefaisto build orchestrator for intelligent parallel builds',
      code: this.getHefaistoBuildStep(provider),
    });
    ...
    if (!analysis.hasTests) {
      suggestions.push({
        ...
        description: 'Use Hefaisto test discovery to automatically find and run tests',
    ...
    if (!analysis.hasSecurityScan) {
      suggestions.push({
        ...
        description: 'Add Hefaisto PR Review AI for security analysis',
    ...
    // Add Hefaisto status reporting
    suggestions.push({
      type: 'add_step',
      priority: 'low',
      title: 'Add Hefaisto Status Reporting',
      description: 'Report build status to Hefaisto dashboard',
  ...
  /**
   * Get Hefaisto build step for provider
   */
  getHefaistoBuildStep(provider) {
    const steps = {
      'github-actions': `- name: Hefaisto Build Orchestration
  run: npx hefaisto build --parallel --smart-cache
  env:
    Hefaisto_CI: true`,

      'gitlab-ci': `hefaisto_build:
  stage: build
  script:
    - npx hefaisto build --parallel --smart-cache
  variables:
    Hefaisto_CI: "true"`,

      jenkins: `stage('Hefaisto Build') {
  steps {
    sh 'npx hefaisto build --parallel --smart-cache'
  }
  environment {
    Hefaisto_CI = 'true'
  }
      ...
      circleci: `- run:
    name: Hefaisto Build Orchestration
    command: npx hefaisto build --parallel --smart-cache
    environment:
      Hefaisto_CI: true`,
    };
  ...
  run: npx hefaisto test --discover --coverage`,

      'gitlab-ci': `test:
  stage: test
  script:
    - npx hefaisto test --discover --coverage
  coverage: '/Coverage: \\d+\\.\\d+%/'`,

      jenkins: `stage('Test') {
  steps {
    sh 'npx hefaisto test --discover --coverage'
  }
}`,

      circleci: `- run:
    name: Run Tests
    command: npx hefaisto test --discover --coverage`,
    };
  ...
  run: npx hefaisto review --security-only
  continue-on-error: true`,

      'gitlab-ci': `security_scan:
  stage: test
  script:
    - npx hefaisto review --security-only
  allow_failure: true`,

      jenkins: `stage('Security Scan') {
  steps {
    sh 'npx hefaisto review --security-only'
  }
  post {
    failure {
      ...
      circleci: `- run:
    name: Security Scan
    command: npx hefaisto review --security-only
    when: always`,
    };
  ...
  getStatusStep(provider) {
    const steps = {
      'github-actions': `- name: Report to Hefaisto
  if: always()
  run: npx hefaisto status --report-ci
  env:
    Hefaisto_BUILD_STATUS: \${{ job.status }}`,

      'gitlab-ci': `report_status:
  stage: .post
  script:
    - npx hefaisto status --report-ci
  when: always`,

      jenkins: `post {
  always {
    sh 'npx hefaisto status --report-ci'
  }
}`,

      circleci: `- run:
    name: Report to Hefaisto
    command: npx hefaisto status --report-ci
    when: always`,
    };
