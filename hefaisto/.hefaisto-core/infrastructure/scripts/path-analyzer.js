...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .hefaisto-core
  { pattern: /`\.hefaisto-core\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.hefaisto-core/development/tasks',
  template: '.hefaisto-core/product/templates',
  checklist: '.hefaisto-core/product/checklists',
  script: '.hefaisto-core/infrastructure/scripts',
  data: '.hefaisto-core/development/data',
  schema: '.hefaisto-core/schemas',
  agent: '.hefaisto-core/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.hefaisto-core/development/agents', type: 'agent' },
    { dir: '.hefaisto-core/development/tasks', type: 'task' },
    { dir: '.hefaisto-core/product/templates', type: 'template' },
    { dir: '.hefaisto-core/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.hefaisto-core/')) {
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  Hefaisto Path Analysis Report');
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.hefaisto-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .hefaisto-core directory. Run from project root.');
    process.exit(1);
  }
