...
/**
 * Hefaisto Plan Progress Tracker
 *
 ...
 *
 * Features:
 * - AC1: Located in `.hefaisto-core/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.hefaisto/dashboard/status.json`
 *
...
const CONFIG = {
  ...
  dashboardStatusPath: '.hefaisto/dashboard/status.json',
  // Legacy status path for backwards compatibility
  legacyStatusPath: '.hefaisto/status.json',
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .hefaisto plans location
      path.join(this.rootPath, '.hefaisto/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., hefaisto-core-ade/story-4.6)
   ...
   *
   * Updates `.hefaisto/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.hefaisto/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - Hefaisto Execution Engine (Story 4.6)
