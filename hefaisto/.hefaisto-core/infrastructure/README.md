...
Base layer of the Hefaisto modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - Hefaisto component generation
- `BatchCreator` - Batch operations

### Validation
- `HefaistoValidator` - Hefaisto component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.hefaisto-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.hefaisto-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.hefaisto-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.hefaisto-core/infrastructure');
