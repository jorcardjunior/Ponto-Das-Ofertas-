...
class JiraAdapter extends PMAdapter {
   ...
   * @param {string} config.project_key - Project key (e.g., "Hefaisto")
   */
  constructor(config) {
    super(config);
  ...
  async pullStory(storyId) {
    try {
      ...
      // Map Jira status to Hefaisto status
      const statusMapping = {
        'To Do': 'Draft',
  ...
  async updateStatus(storyId, status) {
    try {
      ...
      // Map Hefaisto status to Jira transition
   ...
   * Get Jira issue
   * @private
   * @param {string} issueKey - Issue key (e.g., "Hefaisto-123")
