...
- PM tool configured in `.hefaisto-pm-config.yaml` (or will use local-only mode)
...
```javascript
const { getPMAdapter } = require('../.hefaisto-core/scripts/pm-adapter-factory');
...
- **Configuration missing**: Inform user to run `hefaisto init`
...
```javascript
const { syncStoryToPM } = require('../.hefaisto-core/scripts/story-manager');
