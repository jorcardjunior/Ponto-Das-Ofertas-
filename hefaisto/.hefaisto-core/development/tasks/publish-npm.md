...
5. PUBLISH
   ...
   - Verify: npm view hefaisto-core@preview version === {new_version}

6. SMOKE TEST
   - Create temp directory
   - npm init -y
   - npm install hefaisto-core@preview
   - Verify critical files exist in node_modules/hefaisto-core/:
...
8. REPORT
   ...
   - "Test with: npm install hefaisto-core@preview"
...
- [ ] `npm view hefaisto-core@preview` returns new version
...
1. VERIFY PREVIEW EXISTS
   - npm view hefaisto-core@preview version
   - If no preview: HALT with "No preview version found"

2. CONFIRM WITH USER
   - Display: "Promote v{X.Y.Z} from preview to latest?"
   - Show current latest: npm view hefaisto-core@latest version
   - Require explicit confirmation

3. PROMOTE
   - npm dist-tag add hefaisto-core@{version} latest

4. VERIFY
   - npm view hefaisto-core@latest version === {version}
...
6. REPORT
   ...
   - Release URL
   - "Install with: npm install hefaisto-core"
```

#### Pre-conditions

- [ ] Preview version exists (`npm view hefaisto-core@preview`)
...
- [ ] `npm view hefaisto-core@latest` returns promoted version
...
2. INSTALL
   - npm install hefaisto-core@{tag} (default: latest)
   - Record: install time, exit code, warnings

3. VERIFY FILES
   - Check node_modules/hefaisto-core/ contains:
     - .hefaisto-core/core-config.yaml
     - .hefaisto-core/constitution.md
     - .hefaisto-core/development/agents/ (non-empty)
     - .hefaisto-core/development/tasks/ (non-empty)
     - .claude/hooks/synapse-engine.cjs
     - .claude/hooks/precompact-session-digest.cjs
     - .claude/rules/ (non-empty)
     - bin/hefaisto.js
     - bin/hefaisto-minimal.js
   - Check node_modules/hefaisto-core/ does NOT contain:
     - pro/
     - .env
     - .git/
     - tests/
...
4. TEST INSTALLER (optional, if --full flag)
   - npx hefaisto-core install --preset minimal
   - Verify: .hefaisto-core/ created
...
- `--full`: Also test `npx hefaisto-core install`
...
npm view hefaisto-core versions --json

# 2. Point latest back to previous version
npm dist-tag add hefaisto-core@{previous-good-version} latest

# 3. Deprecate broken version with message
npm deprecate hefaisto-core@{broken-version} "Known issues, use v{previous-good-version}"
...
To fully block a version, use `npm unpublish hefaisto-core@{version}` (within 72h of publish only).
