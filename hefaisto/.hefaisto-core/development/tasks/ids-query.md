...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .hefaisto-core/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
```bash
# Basic query
node bin/hefaisto-ids.js ids:query "validate story drafts"

# With JSON output
node bin/hefaisto-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/hefaisto-ids.js ids:query "database migration" --type script

# Filter by category
node bin/hefaisto-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.hefaisto-core/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.hefaisto-core/core/ids/incremental-decision-engine'));
...
- `hefaisto ids:create-review` — Review CREATE decisions for 30-day assessment
