...
**Purpose:** Analyze an existing Hefaisto project to understand its structure, services, patterns, and provide recommendations for implementing new features. This is Phase 1 of the Incremental Feature Workflow.
...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .hefaisto-core/
...
```yaml
pre-conditions:
  - [ ] Project has .hefaisto-core/ directory
    type: pre-condition
    blocker: true
    validation: |
      Check .hefaisto-core/ directory exists in project root
    error_message: "Pre-condition failed: Not an Hefaisto project (.hefaisto-core/ not found)"
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .hefaisto-core/ configuration analyzed
...
1. **Error:** No .hefaisto-core/ Directory
   - **Cause:** Not an Hefaisto project
   - **Resolution:** Initialize Hefaisto first or check directory
...
```javascript
// Core Hefaisto structure
const scanLocations = {
  hefaistoCore: '.hefaisto-core/',
  services: '.hefaisto-core/infrastructure/services/',
  squads: '.hefaisto-core/squads/',
  agents: '.hefaisto-core/development/agents/',
  tasks: '.hefaisto-core/development/tasks/',
  data: '.hefaisto-core/data/'
};
```
...
const configPatterns = {
  ...
  configFile: exists('hefaisto.config.js') || exists('.hefaisto-core/core-config.yaml'),
...
```
.hefaisto-core/infrastructure/services/{service-name}/
...
```
.hefaisto-core/infrastructure/services/{service_name}/
├── README.md
├── index.ts
...
```javascript
const { isCodeIntelAvailable } = require('.hefaisto-core/core/code-intel');
const { getDependencyGraph, getComplexityAnalysis } = require('.hefaisto-core/core/code-intel/helpers/planning-helper');
