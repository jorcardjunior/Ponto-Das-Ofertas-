...
- **Tool:** project-scaffolder
  ...
  - **Source:** .hefaisto-core/scripts/project-scaffolder.js

- **Tool:** config-manager
  - **Purpose:** Initialize configuration files
  - **Source:** .hefaisto-core/utils/config-manager.js
...
**Action:** Read `.hefaisto-core/core-config.yaml` and check `projectStatus.enabled`

**Logic:**
```javascript
const config = yaml.load(fs.readFileSync('.hefaisto-core/core-config.yaml'));
...
projectStatus:
  ...
  components:
    ...
    currentEpic: true
    currentStory: true
  statusFile: .hefaisto/project-status.yaml
  maxModifiedFiles: 5
  maxRecentCommits: 2
```
...
### Step 4: Create .hefaisto Directory

**Action:** Ensure `.hefaisto/` directory exists
...
```javascript
const { loadProjectStatus } = require('./.hefaisto-core/scripts/project-status-loader.js');
const status = await loadProjectStatus();
```

**Verification:** Check that `.hefaisto/project-status.yaml` was created with valid content.
...
**Confirmation:**
```
✅ Initialized project status cache (.hefaisto/project-status.yaml)
```
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.hefaisto-core/scripts/project-status-loader.js');
const status = await loadProjectStatus();
...
**Action:** Ensure `.hefaisto/project-status.yaml` is gitignored

**Check:** Look for `.hefaisto/project-status.yaml` entry in `.gitignore`

**If missing:** Add entry to `.gitignore`

```gitignore
# Hefaisto Project Status Cache (auto-generated)
.hefaisto/project-status.yaml
```

**Confirmation:**
```
✅ Added .hefaisto/project-status.yaml to .gitignore
```
...
Configuration:
  ...
  • Cache file: .hefaisto/project-status.yaml
...
- `.hefaisto/project-status.yaml` - Status cache file (gitignored)

### Files Modified

- `.hefaisto-core/core-config.yaml` - projectStatus section enabled (if was disabled)
...
- [ ] `.hefaisto/project-status.yaml` exists and contains valid YAML
- [ ] `core-config.yaml` has `projectStatus.enabled: true`
- [ ] `.gitignore` includes `.hefaisto/project-status.yaml`
...
**Error:**
```
❌ Could not find .hefaisto-core/core-config.yaml
   Are you in the project root directory?
```

**Resolution:** Navigate to project root before running task.

### Permission Denied on .hefaisto Directory

**Error:**
```
❌ Cannot create .hefaisto directory: Permission denied
```
...
2. **Remove cache file:**
   ```bash
   rm .hefaisto/project-status.yaml
   ```
...
- `.hefaisto-core/scripts/project-status-loader.js` - Core status loader
...
- **Documentation:** `docs/guides/project-status-feature.md`
- **Config:** `.hefaisto-core/core-config.yaml` (projectStatus section)
