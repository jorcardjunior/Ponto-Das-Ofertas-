...
- campo: gotchas_file
  tipo: string
  destino: .hefaisto/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .hefaisto/gotchas.json
  persistido: true
...
```yaml
pre-conditions:
  - [ ] Project has .hefaisto directory initialized
    tipo: pre-condition
    blocker: false
    validação: |
      Check if .hefaisto/ directory exists, create if not
    error_message: "Creating .hefaisto/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .hefaisto/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .hefaisto/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.hefaisto/gotchas.md` and `.hefaisto/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.hefaisto-core/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.hefaisto/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.hefaisto/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.hefaisto-core/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.hefaisto-core/infrastructure/scripts/gotchas-documenter');
...
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .hefaisto-core/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
3. **Error:** Write permission denied
   - **Cause:** Cannot write to .hefaisto directory
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .hefaisto-core/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.hefaisto-core/development/tasks/capture-session-insights.md` - Provides input insights
- `.hefaisto-core/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.hefaisto-core/development/tasks/spec-write-spec.md` - May reference gotchas
