...
const configPath = path.join(__dirname, '../../.hefaisto-core/core-config.yaml');
...
mkdir -p .backups
cp -r .hefaisto-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .hefaisto-core/agents .hefaisto-core/tasks .hefaisto-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .hefaisto-core/agents/ .hefaisto-core/tasks/ .hefaisto-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .hefaisto-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .hefaisto-core/utils-archive
```

**3.2 Create Archive README**

Create `.hefaisto-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .hefaisto-core/utils-archive/utility-name.js .hefaisto-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .hefaisto-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .hefaisto-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .hefaisto-core/utils
```

**Selective Restoration**:
```bash
cp .hefaisto-core/utils-archive/specific-utility.js .hefaisto-core/scripts/
```
...
- **Tool:** task-runner
  ...
  - **Source:** .hefaisto-core/core/task-runner.js

- **Tool:** logger
  - **Purpose:** Execution logging and error tracking
  - **Source:** .hefaisto-core/utils/logger.js
...
- **Script:** execute-task.js
  ...
  - **Language:** JavaScript
  - **Location:** .hefaisto-core/scripts/execute-task.js
...
git mv .hefaisto-core/scripts/utility-name.js .hefaisto-core/utils-archive/

# Or for multiple files:
git mv .hefaisto-core/scripts/{hefaisto-validator-fixed.js,hefaisto-validator-refactored.js} .hefaisto-core/utils-archive/
```
...
git mv .hefaisto-core/scripts/hefaisto-validator.test.js tests/utils/
```
...
1. hefaisto-validator-fixed.js - Duplicate of hefaisto-validator.js
2. hefaisto-validator-refactored.js - Duplicate of hefaisto-validator.js
...
...
Update utility count in `.hefaisto-core/core-config.yaml`:
...
framework:
  entities:
    utils:
      ...
      location: .hefaisto-core/scripts/
```
...
```bash
node .hefaisto-core/scripts/hefaisto-validator.js
```
...
for util in $(ls .hefaisto-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .hefaisto-core/agents .hefaisto-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .hefaisto-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .hefaisto-core/utils
git checkout .hefaisto-core/core-config.yaml
```
...
1. `.hefaisto-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.hefaisto-core/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.hefaisto-core/core-config.yaml` - Corrected utility count
...
- ✅ Framework validation (hefaisto-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
