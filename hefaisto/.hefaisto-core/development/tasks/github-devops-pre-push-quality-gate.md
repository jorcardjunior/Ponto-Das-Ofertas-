...
- Repository context detected (run `hefaisto init` if needed)
...
if (!context) {
  ...
  console.error('Run "hefaisto init" to configure installation mode');
  process.exit(1);
}
...
```javascript
const { assessPrePushImpact, classifyRiskLevel } = require('.hefaisto-core/core/code-intel/helpers/devops-helper');
