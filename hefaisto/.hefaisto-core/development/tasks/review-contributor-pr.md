...
# CI/Workflow = .github/
# Test = tests/
# Code = packages/, .hefaisto-core/, bin/
...
#### For Code PRs (packages/, .hefaisto-core/, bin/)
...
gh pr diff {pr_number} -- 'packages/' '.hefaisto-core/' 'bin/' | grep -E "(shell:\s*true|execSync\(|\.exec\(|eval\(|Function\()"
