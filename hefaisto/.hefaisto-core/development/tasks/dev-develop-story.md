...
} = require('./.hefaisto-core/scripts/decision-recorder');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.hefaisto-core/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.hefaisto-core/core/code-intel/helpers/dev-helper`
...
7. Execute `.hefaisto-core/product/checklists/story-dod-checklist.md`
...
│    │    wsl bash -c 'cd /mnt/c/.../hefaisto-core &&    │   │
...
- `.hefaisto-core/product/checklists/story-dod-checklist.md` - Definition of Done checklist
