...
Load Hefaisto project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .hefaisto-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .hefaisto-core/core-config.yaml
...
```javascript
const { loadContext } = require('.hefaisto-core/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.hefaisto/gotchas.json` | Known issues and workarounds |
| `.hefaisto-core/data/technical-preferences.md` | User-defined patterns |
| `.hefaisto-core/core-config.yaml` | Project configuration |
