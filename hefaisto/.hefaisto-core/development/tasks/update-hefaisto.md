# Task: Update Hefaisto Framework

> **Version:** 4.0.0
> **Created:** 2026-01-29
> **Updated:** 2026-01-31
> **Type:** SYNC (git-native framework synchronization)
> **Agent:** @devops (Gage) or @hefaisto (Orion)
> **Execution:** Simple bash script (~15 lines)

## Purpose

Git-native sync of Hefaisto framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .hefaisto-core/scripts/update-hefaisto.sh

# Review changes shown by the script, then:
git add .hefaisto-core && git commit -m "chore: sync Hefaisto framework"   # Apply changes
# OR
git checkout -- .hefaisto-core/                                         # Cancel changes
```
...
1. **Clone upstream** - Sparse shallow clone of SynkraAI/hefaisto-core (only `.hefaisto-core/`)
...
| Path | Reason |
|------|--------|
| `.hefaisto-core/squads/` | Custom copywriters, data, ralph |
| `.hefaisto-core/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.hefaisto-core/context/` | Compiled contexts |
...
```yaml
task: updateHefaistoFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .hefaisto-core/scripts/update-hefaisto.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .hefaisto-core/scripts/update-hefaisto.sh
  3. Review changes displayed
  4. git add .hefaisto-core && git commit -m "chore: sync Hefaisto framework"  # to apply
  5. git checkout -- .hefaisto-core/                                        # to cancel
...
ls -la .hefaisto-core/squads/  # if exists
