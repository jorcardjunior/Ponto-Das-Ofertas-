...
- **Tool:** ast-parser
  ...
  - **Source:** .hefaisto-core/utils/ast-parser.js
...
- **Script:** modify-file.js
  ...
  - **Language:** JavaScript
  - **Location:** .hefaisto-core/scripts/modify-file.js
...
- Target workflow must exist (path resolved from target_context):
  - `core` → `.hefaisto-core/development/workflows/`
...
- Resolve workflow path based on target_context:
  - `core` → `.hefaisto-core/development/workflows/{workflow-name}.yaml`
...
- Create timestamped backup in same context:
  - `core` → `.hefaisto-core/development/workflows/.backups/{workflow-name}.yaml.{timestamp}`
...
+ modified_by: hefaisto-developer
