...
/**
 * Default Hefaisto minimum version
 * @constant {string}
 */
const DEFAULT_Hefaisto_MIN_VERSION = '2.1.0';
...
class SquadGeneratorError extends Error {
  ...
  static schemaNotFound(schemaPath) {
    return new SquadGeneratorError(
      ...
      'Ensure Hefaisto is properly installed',
    );
  }
}
...
hefaisto:
  minVersion: "${config.hefaistoMinVersion || DEFAULT_Hefaisto_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom Hefaisto squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core Hefaisto configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core Hefaisto coding standards.
...
- Node.js >= 18.x
- Hefaisto >= ${config.hefaistoMinVersion || DEFAULT_Hefaisto_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.hefaistoMinVersion] - Minimum Hefaisto version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      hefaistoMinVersion: config.hefaistoMinVersion || DEFAULT_Hefaisto_MIN_VERSION,
