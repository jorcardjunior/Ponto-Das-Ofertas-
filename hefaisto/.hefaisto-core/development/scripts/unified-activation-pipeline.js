 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or Hefaisto_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
const ALL_AGENT_IDS = [
  ...
  'devops', 'hefaisto-master', 'squad-creator',
];
...
class UnifiedActivationPipeline {
  ...
  async _loadCoreConfig() {
    try {
      const configPath = path.join(this.projectRoot, '.hefaisto-core', 'core-config.yaml');
   ...
   * Priority: Hefaisto_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.Hefaisto_PIPELINE_TIMEOUT;
    if (envTimeout) {
