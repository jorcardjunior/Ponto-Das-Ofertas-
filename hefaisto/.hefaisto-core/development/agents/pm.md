...
IDE-FILE-RESOLUTION:
  ...
  - Example: create-doc.md → .hefaisto-core/development/tasks/create-doc.md
...
activation-instructions:
  ...
  - STEP 2.5: |
      ...
      Check user_profile using config-resolver's resolveConfig():
        ...
        - If user_profile === 'bob':
          → Load bob-orchestrator.js module from .hefaisto-core/core/orchestration/bob-orchestrator.js
      ...
      Module: .hefaisto-core/core/config/config-resolver.js
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .hefaisto-core/development/scripts/unified-activation-pipeline.js pm
  - STEP 3.5: |
      ...
      1. Run data lifecycle cleanup first:
         - const { runStartupCleanup } = require('.hefaisto-core/core/orchestration/data-lifecycle-manager')
         - await runStartupCleanup(projectRoot) // Cleanup locks, sessions >30d, snapshots >90d

      2. Check for existing session state:
         - const { BobOrchestrator } = require('.hefaisto-core/core/orchestration/bob-orchestrator')
      ...
      Module: .hefaisto-core/core/orchestration/bob-orchestrator.js (Story 12.5)
      Module: .hefaisto-core/core/orchestration/data-lifecycle-manager.js (Story 12.5)
      Task: .hefaisto-core/development/tasks/session-resume.md
...
persona:
  ...
  orchestration_constraints:
    ...
    integration:
      module: .hefaisto-core/core/orchestration/terminal-spawner.js
      script: .hefaisto-core/scripts/pm.sh
      executor_assignment: .hefaisto-core/core/orchestration/executor-assignment.js
...
commands:
  ...
  # NOTE: correct-course removed - delegated to @hefaisto-master
  # See: docs/architecture/command-authority-matrix.md
  # For course corrections → Escalate to @hefaisto-master using *correct-course
...
- Course corrections → Escalate to @hefaisto-master using `*correct-course`
...
| Course correction | @hefaisto-master | `*correct-course` |
...
| @hefaisto-master | Framework modification | `*create-brownfield-prd` |
...
2. PRD templates in `.hefaisto-core/product/templates/`
...
6. **Course correction** → Escalate to `@hefaisto-master *correct-course` if deviations detected
