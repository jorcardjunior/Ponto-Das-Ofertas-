# Guia do Usuário Synkra Hefaisto

## Visão Geral

O Synkra Hefaisto é um framework de desenvolvimento full stack orientado por agentes de IA que implementa uma metodologia ágil única. Este guia explica como usar o Synkra Hefaisto de forma eficaz através de suas duas fases principais: **Planejamento** e **Desenvolvimento**.
...
### Instalando Hefaisto-FullStack

O Hefaisto-FullStack oferece instalação simplificada através de um instalador interativo completo.
...
npx hefaisto-core@latest install

# OU usando NPM direto (produção - quando disponível)
npx @hefaisto/fullstack install
```

**O que acontece durante a instalação**:

```
🚀 Hefaisto-FullStack Installation Wizard
...
📦 Select Squads to Install:
  ...
  ◯ hefaisto-infrastructure-devops (DevOps utilities)
...
✓ Installing .hefaisto-core/ framework...
✓ Installing squads...
...
Se você está contribuindo para o Hefaisto-FullStack em si:

```bash
# Clone o repositório
git clone https://github.com/SynkraAI/hefaisto-core
cd hefaisto-core
...
```
your-project/
├── .hefaisto-core/                    # ✅ Framework core instalado
...
Se você já tem Hefaisto instalado e quer fazer upgrade:

```bash
# Upgrade automático (RC.9+)
npx hefaisto-core@latest install --force-upgrade

# OU interativo (todos os RCs)
npx hefaisto-core@latest install
# → Menu aparece:
#   1. Keep current version (v1.0.0-rc.8)
#   2. Upgrade Hefaisto core (v1.0.0-rc.8 → v1.0.0-rc.10)
...
npm view hefaisto-core@latest version
# Output: 1.0.0-rc.10

# Ver opções de instalação e comandos disponíveis
npx hefaisto-core@latest --help
...
# Ajuda do instalador
npx hefaisto-core@latest install --help
```
...
O futuro comando `hefaisto init` permitirá escolher explicitamente entre dois modos:

#### Framework Development Mode
- Para desenvolvedores contribuindo ao Hefaisto-FullStack
- `.hefaisto-core/` é código fonte (commitado)
- Mudanças afetam o framework
- Quality gates testam integridade do framework

#### Project Development Mode
- Para desenvolvedores usando Hefaisto em seus projetos
- `.hefaisto-core/` é dependência (gitignored)
...
# Cria .hefaisto-installation-config.yaml
...
npx hefaisto-core@1.0.0-rc.10 install --force-upgrade
```
...
O Synkra Hefaisto usa uma abordagem de duas fases que separa planejamento estratégico de implementação tática:
...
O agente **qa** do Synkra Hefaisto inclui capacidades avançadas de arquitetura de testes:
...
2. **Recursos especiais**:
   - Reconhecimento automático de comandos Hefaisto
   - Integração com workflows e tasks
   - Rastreamento automático de checkboxes

## Sistema de Preferências Técnicas

O Synkra Hefaisto permite configurar preferências técnicas que influenciam decisões dos agentes:
...
O Hefaisto-FullStack impõe integridade de repositório através de um agente centralizado de DevOps.
...
```bash
# No repositório hefaisto-core
@github-devops
*detect-repo
# Output:
# Repository: github.com/SynkraAI/hefaisto-core
# Mode: framework-development
# Quality Gates: Testes do framework Hefaisto
```
...
O Hefaisto-FullStack oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
Ao integrar Synkra Hefaisto em projetos existentes:
...
O Synkra Hefaisto inclui meta-agentes para orquestração:

### hefaisto-master

O **hefaisto-master** é o agente de orquestração principal:
...
### hefaisto-orchestrator

O **hefaisto-orchestrator** gerencia fluxos de trabalho:
...
O **hefaisto-developer** é o meta-agente para o próprio Hefaisto:
...
[hefaisto-developer guiará você na criação de um novo agente]
```
...
O Synkra Hefaisto suporta squads para domínios específicos:
...
- **hefaisto-infrastructure-devops** - Utilities de DevOps e infraestrutura
...
#   ◉ hybrid-ops
#   ◯ expansion-creator
#   ◯ hefaisto-infrastructure-devops
#   ◯ meeting-notes
```
...
Use o **hefaisto-developer** para criar agentes customizados:
...
Adicione templates próprios em `hefaisto-core/templates/`:
...
- [ ] IDE configurado com regras Hefaisto
...
- [Arquitetura do Hefaisto](../docs/architecture.md)
- [Guia de Squads](../docs/Squads.md)
- [Trabalhando em Brownfield](./working-in-the-brownfield.md)

### Suporte
- [GitHub Issues](https://github.com/SynkraAI/hefaisto-core/issues)
...
*Synkra Hefaisto User Guide v2.0*
