...
function getDefaultOptions() {
  ...
  const envLocalDir = process.env.Hefaisto_CODEX_LOCAL_SKILLS_DIR;
  const envGlobalDir = process.env.Hefaisto_CODEX_GLOBAL_SKILLS_DIR;
  return {
    projectRoot,
    sourceDir: path.join(projectRoot, '.hefaisto-core', 'development', 'agents'),
...
function getSkillId(agentId) {
  ...
  if (id.startsWith('hefaisto-')) return id;
  return `hefaisto-${id}`;
}
...
function buildSkillContent(agentData) {
  ...
  const title = agent.title || 'Hefaisto Agent';
...
# Hefaisto ${title} Activator

## When To Use
${whenToUse}

## Activation Protocol
1. Load \`.hefaisto-core/development/agents/${agentData.filename}\` as source of truth (fallback: \`.codex/agents/${agentData.filename}\`).
2. Adopt this agent persona and command system.
3. Generate greeting via \`node .hefaisto-core/development/scripts/generate-greeting.js ${agentData.id}\` and show it first.
...
## Non-Negotiables
- Follow \`.hefaisto-core/constitution.md\`.
