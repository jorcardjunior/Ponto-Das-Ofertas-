...
// Purpose: Collect tool usage data per session, store in .hefaisto/analytics/,
//          enforce data governance (sanitization, retention, schema).
//
// Usage:
//   node .hefaisto-core/infrastructure/scripts/collect-tool-usage.js [options]
//
// Options:
...
// --- Configuration ---
const ANALYTICS_DIR = path.resolve(process.cwd(), '.hefaisto', 'analytics');
...
function loadUsageData() {
  ...
  return {
    ...
    description: 'Tool usage tracking data for Hefaisto token optimization (TOK-5)',
    sessions: []
  };
}
