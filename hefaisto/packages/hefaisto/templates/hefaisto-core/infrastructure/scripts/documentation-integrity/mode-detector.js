 ...
 *
 * Detects installation mode for Hefaisto projects with three-mode support:
 * - FRAMEWORK_DEV: Contributing to hefaisto-core itself
...
/**
 * Installation modes supported by Hefaisto
 * @enum {string}
 */
...
const LegacyProjectType = {
  EXISTING_Hefaisto: 'EXISTING_Hefaisto',
  GREENFIELD: 'GREENFIELD',
  BROWNFIELD: 'BROWNFIELD',
  UNKNOWN: 'UNKNOWN',
};
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing hefaisto-core itself - uses framework standards, skips project setup',
    description: 'For Hefaisto contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with Hefaisto - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add Hefaisto to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .hefaisto-core/ exists AND is hefaisto-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for Hefaisto framework development
  if (markers.hasHefaistoCore && markers.isHefaistoCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_Hefaisto,
      confidence: 100,
      reason: 'Detected hefaisto-core repository with .hefaisto-core directory',
      markers,
    };
  }

  // Priority 2: Check for existing Hefaisto installation (non-framework)
  if (markers.hasHefaistoCore && !markers.isHefaistoCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_Hefaisto,
      confidence: 95,
      reason: 'Hefaisto already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // Hefaisto markers
    hasHefaistoCore: fs.existsSync(path.join(targetDir, '.hefaisto-core')),
    isHefaistoCoreRepo: isHefaistoCoreRepository(targetDir),
...
/**
 * Checks if the target directory is the hefaisto-core repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the hefaisto-core repository
 */
function isHefaistoCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit hefaisto-core package names
    if (packageJson.name === '@hefaisto/core' || packageJson.name === 'hefaisto-core') {
      return true;
    }

    // Secondary check: workspaces pattern + hefaisto-specific marker file
    // This prevents false positives for generic monorepos
    const hasHefaistoMarker = fs.existsSync(path.join(targetDir, '.hefaisto-core', 'infrastructure'));
    const hasWorkspaces =
      Array.isArray(packageJson.workspaces) && packageJson.workspaces.includes('packages/*');

    return hasWorkspaces && hasHefaistoMarker;
  } catch (error) {
    // Log error for debugging but don't throw - return false for safety
    if (process.env.Hefaisto_DEBUG) {
      console.warn(`[mode-detector] Error checking hefaisto-core repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_Hefaisto legacy type (both use EXISTING_Hefaisto but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_Hefaisto, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isHefaistoCoreRepo] - True if this is the hefaisto-core repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_Hefaisto is ambiguous: it can be FRAMEWORK_DEV (hefaisto-core repo)
  // or BROWNFIELD (user project with Hefaisto installed)
  if (legacyType === LegacyProjectType.EXISTING_Hefaisto) {
    // Use context to disambiguate if provided
    if (context.isHefaistoCoreRepo === true) {
      return InstallationMode.FRAMEWORK_DEV;
    }
    if (context.isHefaistoCoreRepo === false) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isHefaistoCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the hefaisto-core repository.',
      );
    }
