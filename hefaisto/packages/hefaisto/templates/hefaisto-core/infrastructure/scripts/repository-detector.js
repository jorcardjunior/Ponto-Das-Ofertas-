 ...
 * @property {string} frameworkLocation - Path to Hefaisto framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in hefaisto-core repo itself
  const isFrameworkRepo =
    packageJson.name === '@hefaisto/fullstack' ||
    packageJson.name === 'hefaisto-core' ||
    remoteUrl.includes('hefaisto-core');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.hefaisto-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@hefaisto/fullstack'),
