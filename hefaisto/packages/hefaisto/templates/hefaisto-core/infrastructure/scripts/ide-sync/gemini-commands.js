...
const FALLBACK_DESCRIPTION = 'Agente especializado Hefaisto';
...
function commandSlugForAgent(agentId) {
  if (agentId.startsWith('hefaisto-')) {
    return agentId.replace(/^hefaisto-/, '');
  }
  return agentId;
}

function menuCommandName(agentId) {
  return `/hefaisto-${commandSlugForAgent(agentId)}`;
}
...
function buildAgentDescription(agent) {
  ...
  return `Ativar agente Hefaisto ${agent.id}`;
}

function buildAgentCommandPrompt(agentId) {
  return [
    `Ative o agente ${agentId}:`,
    `1. Leia a definição completa em .gemini/rules/Hefaisto/agents/${agentId}.md`,
    '2. Siga as activation-instructions do bloco YAML',
    `3. Renderize o greeting via: node .hefaisto-core/development/scripts/generate-greeting.js ${agentId}`,
...
function buildMenuPrompt(commandFiles) {
  const lines = [
    'Você está no launcher Hefaisto para Gemini.',
    '',
...
function buildMenuCommandFile(commandFiles) {
  const content = [
    'description = "Menu rápido Hefaisto (lista agentes e orienta qual ativar)"',
    'prompt = """',
