...
const CORE_CONFIG_FILE = '.hefaisto-core/core-config.yaml';
 ...
 * - .hefaisto-core/core/** gets expanded one level deep
...
function expandProtectedPaths(protectedPaths, exceptions, projectRoot) {
  ...
  for (const globPath of protectedPaths) {
    if (globPath === '.hefaisto-core/core/**') {
      const { dirs, files } = expandOneLevel('.hefaisto-core/core', projectRoot);
