/**
 * Hefaisto Skill Validator
 * Story GEMINI-INT.5 - Skills Cross-CLI Compatibility
 *
 * Validates that Hefaisto agent skills work correctly in both
...
class SkillValidator {
  constructor(config = {}) {
    this.agentsDir = config.agentsDir || path.join(process.cwd(), '.hefaisto-core', 'development', 'agents');
    this.tasksDir = config.tasksDir || path.join(process.cwd(), '.hefaisto-core', 'development', 'tasks');
    this.errors = [];
    this.warnings = [];
  }
  ...
  validateGeminiCompatibility(agentDef, result) {
    // Gemini CLI uses .gemini/rules/Hefaisto/agents/ structure
  ...
  generateReport(results) {
    let report = '# Hefaisto Skills Cross-CLI Compatibility Report\n\n';
