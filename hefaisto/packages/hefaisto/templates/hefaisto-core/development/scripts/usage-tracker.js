...
/**
 * Usage tracker for Hefaisto-FULLSTACK framework components
 * Tracks component usage patterns for deprecation warnings and impact analysis
 */
class UsageTracker {
  constructor(options = {}) {
    this.rootPath = options.rootPath || process.cwd();
    this.usageDir = path.join(this.rootPath, '.hefaisto', 'usage');
  ...
  async getComponentInfo(componentId) {
    ...
    return {
      ...
      type: type,
      name: name,
      file_path: `hefaisto-core/${type}s/${name}.md`
    };
  }

  async getScanPaths(_options) {
    const defaultPaths = [
      path.join(this.rootPath, 'hefaisto-core'),
