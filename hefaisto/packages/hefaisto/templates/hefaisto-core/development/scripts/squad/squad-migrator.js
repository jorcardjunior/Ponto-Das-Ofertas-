...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing hefaisto.type
      if (!manifest.hefaisto?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_Hefaisto_TYPE',
          message: 'Missing required field: hefaisto.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
      ...
      // Check for missing hefaisto.minVersion
      if (!manifest.hefaisto?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: hefaisto.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'hefaisto.minVersion',
          value: '2.1.0',
        });
      }
