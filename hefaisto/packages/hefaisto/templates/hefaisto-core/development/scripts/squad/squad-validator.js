      ...
      try {
        ...
        if (!hasYamlFrontmatter && !hasMarkdownHeading) {
          result.warnings.push({
            ...
            file: agentFile,
            message: 'Agent file may not follow Hefaisto agent definition format',
            suggestion:
  ...
  async validateWorkflows(squadPath) {
    ...
    try {
      ...
      result.warnings.push({
        ...
        suggestion: 'Ensure workflow-validator.js exists in .hefaisto-core/development/scripts/',
      });
      return result;
    }

    const coreAgentsPath = path.join(process.cwd(), '.hefaisto-core', 'development', 'agents');
