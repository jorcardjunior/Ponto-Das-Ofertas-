 ...
 *
 * Downloads squads from the hefaisto-squads GitHub repository.
...
/**
 * Default registry URL for hefaisto-squads
 * @constant {string}
 */
const REGISTRY_URL =
  'https://raw.githubusercontent.com/SynkraAI/hefaisto-squads/main/registry.json';

/**
 * GitHub API base URL for hefaisto-squads contents
 * @constant {string}
 */
const GITHUB_API_BASE =
  'https://api.github.com/repos/SynkraAI/hefaisto-squads/contents/packages';
...
/**
 * Squad Downloader class for downloading squads from hefaisto-squads repository
 */
class SquadDownloader {
  ...
  /**
   * Fetch registry from hefaisto-squads repository
   *
  ...
  async _downloadSquadFiles(squadInfo, targetPath) {
    ...
    if (!Array.isArray(contents)) {
      throw new SquadDownloaderError(
        ...
        'Check if squad exists in hefaisto-squads repository',
      );
    }
  ...
  _fetch(url, useApi = false) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'Hefaisto-SquadDownloader/1.0',
        },
      };
