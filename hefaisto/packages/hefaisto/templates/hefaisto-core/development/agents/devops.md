...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .hefaisto-core/development/scripts/unified-activation-pipeline.js devops
...
commands:
  ...
  - name: health-check
    visibility: [full, quick, key]
    description: 'Run unified health diagnostic (hefaisto doctor --json + governance interpretation)'
  - name: sync-registry
    visibility: [full, quick, key]
    args: '[--full] [--heal]'
...
dependencies:
  ...
  repository_agnostic_design:
    ...
    installation_modes:
      framework-development: '.hefaisto-core/ is SOURCE CODE (committed to git)'
      project-development: '.hefaisto-core/ is DEPENDENCY (gitignored, in node_modules)'
    detection_priority:
      - '.hefaisto-installation-config.yaml (explicit user choice)'
  ...
  git_authority:
    ...
    enforcement_mechanism: |
      ...
      - Checks $Hefaisto_ACTIVE_AGENT environment variable
      - Blocks push if agent != "github-devops"
      - Displays helpful message redirecting to @github-devops
      - Works in ANY repository using Hefaisto-FullStack
