...
if (!context) {
  throw new Error('Unable to detect repository. Run "hefaisto init" first.');
}
```
...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.hefaisto-core', 'core-config.yaml');
...
```javascript
const { generateImpactSummary } = require('.hefaisto-core/core/code-intel/helpers/devops-helper');
...
### NPM Package with Semantic-Release (hefaisto-core)
```yaml
github:
  pr:
