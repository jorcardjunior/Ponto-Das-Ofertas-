...
Read the current permission mode from `.hefaisto/config.yaml` under `permissions.mode`.
...
Update `.hefaisto/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.hefaisto/` directory if needed.
...
```javascript
const { cycleMode } = require('./.hefaisto-core/core/permissions');
...
- If `.hefaisto/config.yaml` cannot be read, start from default `ask` mode
...
version: 1.0.0
dependencies:
  - .hefaisto-core/core/permissions/permission-mode.js
  - .hefaisto-core/core/permissions/index.js
tags:
  - permissions
  - mode-toggle
  - universal-command
