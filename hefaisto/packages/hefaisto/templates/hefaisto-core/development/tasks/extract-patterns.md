...
outputs:
  - name: patterns_markdown
    type: string
    destino: File (.hefaisto/patterns.md)
    persistido: true
...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .hefaisto-core exists
...
```javascript
const PatternExtractor = require('.hefaisto-core/infrastructure/scripts/pattern-extractor');
...
Subcommands:
  ...
  save        Save to .hefaisto/patterns.md
...
Examples:
  ...
  *extract-patterns save                      # Save to .hefaisto/patterns.md
...
Output File:
  Default location: .hefaisto/patterns.md
...
Patterns saved to: .hefaisto/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .hefaisto/patterns.md exists and is valid markdown
...
node .hefaisto-core/infrastructure/scripts/pattern-extractor.js [command] [options]

# Via Hefaisto command
...
dependencies:
  modules:
    - .hefaisto-core/infrastructure/scripts/pattern-extractor.js
  tasks: []
  referenced_by:
    - spec-write.md
tags:
