...
- **Tool:** code-analyzer
  ...
  - **Source:** .hefaisto-core/utils/code-analyzer.js
...
- **Script:** analyze-codebase.js
  ...
  - **Language:** JavaScript
  - **Location:** .hefaisto-core/scripts/analyze-codebase.js
...
Systematically audit all utilities in `.hefaisto-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .hefaisto-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .hefaisto-core/agents .hefaisto-core/tasks Squads 2>/dev/null | wc -l)
