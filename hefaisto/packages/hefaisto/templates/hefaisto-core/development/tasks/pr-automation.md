...
To help users contribute to the Hefaisto open-source project (`hefaisto-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `hefaisto-core` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"hefaisto-core/agents/improved-agent.md"`
...
- **pr_url**: `string`
  ...
  - **Example**: `"https://github.com/SynkraAI/hefaisto-core/pull/123"`
...
2. **Validate Repository State**
   - Check if `hefaisto-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `hefaisto-core/agents/*` → "agent"
     - `hefaisto-core/tasks/*` → "task"
     - `hefaisto-core/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using Hefaisto PR Automation.
      ```
...
18. **Provide Next Steps**
    - Display to user:
      ...
      Thank you for contributing to Hefaisto! 🚀
      ```
...
- [ ] Fork of hefaisto-core exists
  - **Validation**: `gh repo view {user}/hefaisto-core` succeeds
...
```bash
hefaisto activate Otto  # github-devops agent
hefaisto pr create \
  --type="Squad" \
...
hefaisto pr create \
  --type="agent" \
  --path="hefaisto-core/agents/improved-po.md" \
...
hefaisto pr create \
  --type="bug-fix" \
  --path="hefaisto-core/tasks/create-next-story.md" \
