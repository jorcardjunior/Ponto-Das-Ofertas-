...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the Hefaisto framework.

## Directory Structure

```
.hefaisto-core/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
const { loadBlock, executeBlock } = require('.hefaisto-core/utils/block-loader');
...
| `agent-prompt-template` | Standardized template for spawning Hefaisto agents | Any skill/task that invokes agents via Task tool |
...
*Hefaisto Task Blocks System v1.1.0*
