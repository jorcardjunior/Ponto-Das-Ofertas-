...
Standardized template for spawning Hefaisto agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the Hefaisto {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/Hefaisto/agents/architect.md,
...
```javascript
const { loadBlock, renderTemplate } = require('.hefaisto-core/utils/block-loader');
