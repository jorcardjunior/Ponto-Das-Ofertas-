...
Complete environment bootstrap for new Hefaisto projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
## Task Definition (Hefaisto Task Format V1.0)
...
- campo: environment_report
  tipo: object
  destino: File system (.hefaisto/environment-report.yaml)
  persistido: true
...
post-conditions:
  ...
  - [ ] Environment report generated
    ...
    blocker: false
    validação: |
      Check .hefaisto/environment-report.yaml exists
...
acceptance-criteria:
  ...
  - [ ] Project structure follows Hefaisto conventions
    tipo: acceptance-criterion
    blocker: false
    validação: |
      Check docs/, .hefaisto/, and package.json exist
...
- **Tool:** cli-checker
  ...
  - **Source:** .hefaisto-core/infrastructure/scripts/cli-checker.js

- **Tool:** github-cli
  - **Purpose:** Repository creation and authentication
  - **Source:** .hefaisto-core/infrastructure/tools/cli/github-cli.yaml
...
- Cache detection results in .hefaisto/environment-report.yaml
...
ESSENTIAL (required for Hefaisto):
...
# Hefaisto generated files
.hefaisto/project-status.yaml
.hefaisto/environment-report.yaml
...
---
*Generated by Hefaisto Environment Bootstrap*
...
- Initialize Synkra Hefaisto project structure
...
**Action:** Create Hefaisto-compliant project structure
...
# Create .hefaisto/config.yaml
@"
# Hefaisto Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".hefaisto/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with Synkra Hefaisto",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.hefaisto/user-config.yaml`

**Elicitation Point (PRD §2.4):**

```
🤖 Bem-vindo ao Hefaisto!
...
```bash
# Create ~/.hefaisto/ with secure permissions
mkdir -p ~/.hefaisto
chmod 700 ~/.hefaisto

# Write user-config.yaml with selected profile
cat > ~/.hefaisto/user-config.yaml << EOF
# Hefaisto User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('.hefaisto-core/core/config/config-resolver');
...
- `~/.hefaisto/` directory must have permissions 700
- `~/.hefaisto/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".hefaisto/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .hefaisto/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ Hefaisto ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
╠═══════════════════════════════════════════════════════════════════════════╣
║  my-awesome-project/                                                       ║
║  ├── .hefaisto/                    # Hefaisto configuration                        ║
...
║                                                                            ║
║  Full workflow: .hefaisto-core/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .hefaisto/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • Hefaisto help: @hefaisto-master *help                                           ║
...
- [Hefaisto Greenfield Workflow](.hefaisto-core/development/workflows/greenfield-fullstack.yaml)
