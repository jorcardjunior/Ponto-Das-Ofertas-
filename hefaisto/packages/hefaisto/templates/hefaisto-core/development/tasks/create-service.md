...
outputs:
  - name: service_directory
    type: directory
    location: ".hefaisto-core/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .hefaisto-core/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .hefaisto-core/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for Hefaisto agents
...
const { checkBeforeWriting } = require('.hefaisto-core/core/code-intel/helpers/dev-helper');
...
// Check uniqueness
const targetDir = `.hefaisto-core/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.hefaisto-core/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .hefaisto-core/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .hefaisto-core/infrastructure/services/{service_name}
