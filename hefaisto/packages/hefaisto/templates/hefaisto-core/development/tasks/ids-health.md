...
```bash
node bin/hefaisto-ids.js ids:health
```
...
```bash
node bin/hefaisto-ids.js ids:health --fix
```
...
- Log all healing actions to `.hefaisto-core/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/hefaisto-ids.js ids:health --json
node bin/hefaisto-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require('.hefaisto-core/core/ids/registry-healer');
