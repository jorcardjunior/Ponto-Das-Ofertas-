...
post-conditions:
  - [ ] Worktree directory removed
    tipo: post-condition
    blocker: true
    validação: Directory .hefaisto/worktrees/{storyId} does not exist
...
Removes an Hefaisto-managed worktree and its associated branch. Includes safety checks for uncommitted changes and provides options for force removal.
...
Options:
  1. Commit changes first    : cd .hefaisto/worktrees/{storyId} && git commit
...
You are about to remove:
  • Worktree: .hefaisto/worktrees/{storyId}
  • Branch:   auto-claude/{storyId}
...
1. `git worktree remove .hefaisto/worktrees/{storyId}`
...
❌ Cannot remove worktree while inside it.
   ...
   Current directory: .hefaisto/worktrees/{storyId}
...
```bash
# Remove worktree
git worktree remove .hefaisto/worktrees/{storyId} --force
...
- `.hefaisto-core/infrastructure/scripts/worktree-manager.js`
