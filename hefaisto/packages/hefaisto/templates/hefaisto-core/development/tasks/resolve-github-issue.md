...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.hefaisto-core/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.hefaisto-core/development/agents/)
...
steps:
  ...
  3_handle_edge_cases:
    ...
    known_pitfalls:
      - Email addresses inside strings may match rename patterns (Issue #159: security@synkra/hefaisto-core.dev)
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .hefaisto-core/install-manifest.yaml
    when: Any file in .hefaisto-core/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .hefaisto-core/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .hefaisto-core/development/agents/ or ide-sync/
```
...
- **Pitfall:** `@synkra/hefaisto-core` inside email `security@synkra/hefaisto-core.dev` was corrupted
