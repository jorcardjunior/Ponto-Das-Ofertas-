...
# Suggest Refactoring - Hefaisto Developer Task
...
```bash
# Analyze single file
*suggest-refactoring hefaisto-core/scripts/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring hefaisto-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring hefaisto-core/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring hefaisto-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 Hefaisto Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.hefaisto', 'refactoring', `${suggestionId}.json`);
...
6. **Code Intelligence Blast Radius** (if available):
   - Call `assessRefactoringImpact(candidateFiles)` from `.hefaisto-core/core/code-intel/helpers/dev-helper`
