...
> Story Hefaisto-DIFF-4.3.2: Checklist formal de compatibilidade retroativa
...
- [ ] `.env` files backed up (never overwritten by Hefaisto)
...
### 9. Hefaisto Integration
- [ ] `npx hefaisto-core doctor` reports healthy
- [ ] Agent activation works (@dev, @architect, etc.)
- [ ] Existing docs not duplicated

### 10. Rollback Verification
- [ ] `git diff HEAD~1` shows only Hefaisto additions
- [ ] `git checkout HEAD~1 -- .` would restore pre-Hefaisto state
- [ ] No orphaned Hefaisto processes or files

---

## Compatibility Matrix

| Existing Config | Hefaisto Behavior | User Action Required |
|-----------------|---------------|---------------------|
...
# Option 2: Remove only Hefaisto files
rm -rf .hefaisto-core/
rm -rf docs/architecture/ docs/prd/ docs/stories/
# Review and revert .gitignore Hefaisto entries
...
npx hefaisto-core doctor --pre-migration
```

**Post-Migration:**
```bash
# Validate migration
npx hefaisto-core doctor
...
*Hefaisto Brownfield Compatibility Checklist v1.0*
*Story Hefaisto-DIFF-4.3.2*
