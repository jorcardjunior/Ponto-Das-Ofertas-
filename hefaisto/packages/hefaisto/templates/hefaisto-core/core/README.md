# Hefaisto Core Module

> Central runtime module providing essential framework functionality for Synkra Hefaisto.
...
The Core module contains the foundational runtime components that all other Hefaisto modules depend on. It provides configuration management, session handling, elicitation workflows, and essential utilities.

## Installation

The core module is automatically available within the Synkra Hefaisto framework:

```javascript
// CommonJS
const core = require('./.hefaisto-core/core');

// ES Modules
import { ConfigCache, ElicitationEngine } from './.hefaisto-core/core/index.esm.js';
```
...
│   ├── hefaisto-kb.md          # Hefaisto knowledge base
...
```javascript
const { globalConfigCache } = require('./.hefaisto-core/core');
...
```javascript
const { loadAgentConfig } = require('./.hefaisto-core/core');
...
```javascript
const { loadConfigSections } = require('./.hefaisto-core/core');
...
```javascript
const { ContextDetector } = require('./.hefaisto-core/core');
...
```javascript
const { SessionContextLoader } = require('./.hefaisto-core/core');
...
```javascript
const { ElicitationEngine } = require('./.hefaisto-core/core');
...
```javascript
const { ElicitationSessionManager } = require('./.hefaisto-core/core');
...
```javascript
const { YAMLValidator, validateYAML } = require('./.hefaisto-core/core');
...
```javascript
const { PersonalizedOutputFormatter } = require('./.hefaisto-core/core');
...
*Synkra Hefaisto Core Module v2.0.0*
