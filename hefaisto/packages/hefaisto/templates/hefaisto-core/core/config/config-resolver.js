 ...
 */
const CONFIG_FILES = {
  framework: '.hefaisto-core/framework-config.yaml',
  project: '.hefaisto-core/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.hefaisto-core/local-config.yaml',
  legacy: '.hefaisto-core/core-config.yaml',
  user: path.join(os.homedir(), '.hefaisto', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, 'hefaisto-app.config.yaml');
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.hefaisto/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation = process.env.Hefaisto_SUPPRESS_DEPRECATION === 'true'
    || process.env.Hefaisto_SUPPRESS_DEPRECATION === '1';

  if (!suppressDeprecation) {
    warnings.push(
      '[DEPRECATION] Monolithic core-config.yaml detected. '
      + 'Run `hefaisto config migrate` to split into layered config files. '
      + 'Monolithic format will be removed in v4.0.0. '
      + 'Set Hefaisto_SUPPRESS_DEPRECATION=true to silence this warning.',
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case 'app': case '3': case 'L3':
      ...
      relativePath = path.join(options.appDir, 'hefaisto-app.config.yaml');
      break;
...
/**
 * Ensure the ~/.hefaisto/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.hefaisto/user-config.yaml).
