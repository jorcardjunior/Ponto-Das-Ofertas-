 ...
 *
 * @module hefaisto-core/doctor/fix-handler
 * @story INS-4.1
 */
...
const FIX_MAP = {
  ...
  'agent-memory': {
    ...
    async apply(result, { projectRoot }) {
      const agentsDir = path.join(projectRoot, '.hefaisto-core', 'development', 'agents');
      let created = 0;
  ...
  'claude-md': {
    ...
    async apply() {
      return 'CLAUDE.md regeneration requires npx hefaisto-core install --force';
    },
  },
  ...
  'settings-json': {
    ...
    async apply(result, { frameworkRoot }) {
      ...
      return 'settings.json regeneration requires npx hefaisto-core install --force';
    },
  },
};
