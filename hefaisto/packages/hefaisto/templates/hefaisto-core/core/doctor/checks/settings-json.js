 ...
 *
 * @module hefaisto-core/doctor/checks/settings-json
 * @story INS-4.1
 */
...
function checkBoundaryAlignment(context, denyRules) {
  const configPath = path.join(context.projectRoot, '.hefaisto-core', 'core-config.yaml');
...
async function run(context) {
  ...
  if (denyCount < 40) {
    return {
      ...
      fixCommand: 'hefaisto doctor --fix',
    };
  }
