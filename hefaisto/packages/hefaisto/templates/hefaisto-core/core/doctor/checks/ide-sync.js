 ...
 *
 * Validates agents in .claude/commands/Hefaisto/agents/ match
 * .hefaisto-core/development/agents/ (count and names).
 *
 * @module hefaisto-core/doctor/checks/ide-sync
 * @story INS-4.1
 */
...
async function run(context) {
  const agentsSourceDir = path.join(context.projectRoot, '.hefaisto-core', 'development', 'agents');
  const agentsIdeDir = path.join(context.projectRoot, '.claude', 'commands', 'Hefaisto', 'agents');
  ...
  if (!fs.existsSync(agentsSourceDir)) {
    return {
      ...
      fixCommand: 'npx hefaisto-core install --force',
    };
  }
  ...
  if (!fs.existsSync(agentsIdeDir)) {
    return {
      check: name,
      status: 'WARN',
      message: 'IDE agents directory not found (.claude/commands/Hefaisto/agents/)',
      fixCommand: 'npx hefaisto-core install --force',
    };
  }
