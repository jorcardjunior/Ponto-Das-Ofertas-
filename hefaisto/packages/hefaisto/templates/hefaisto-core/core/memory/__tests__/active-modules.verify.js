/**
 * Verification Tests for Active Hefaisto Memory Modules
 *
...
test('merge-rules.yaml exists in .hefaisto', () => {
  const rulesPath = path.join(process.cwd(), '.hefaisto', 'merge-rules.yaml');
  const exists = fs.existsSync(rulesPath);

  assertTrue(exists, 'merge-rules.yaml should exist in .hefaisto');
});
