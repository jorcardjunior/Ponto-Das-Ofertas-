 ...
 */
const DEFAULT_CONFIG_PATH = '.hefaisto-core/core/quality-gates/quality-gate-config.yaml';
...
class QualityGateManager {
   ...
   */
  async getStatus() {
    const statusPath = this.config.status?.location || '.hefaisto/qa-status.json';
  ...
  async saveReport(storyId = 'unknown') {
    const reportDir = this.config.reports?.location || '.hefaisto/qa-reports';
