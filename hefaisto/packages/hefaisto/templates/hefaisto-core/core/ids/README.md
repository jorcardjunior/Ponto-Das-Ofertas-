...
The Entity Registry is the central data store for the Incremental Development System (IDS). It tracks all Hefaisto framework artifacts — tasks, templates, scripts, modules, agents, checklists, and data files — with metadata, relationships, adaptability scores, and checksums.

## Schema

The registry is stored at `.hefaisto-core/data/entity-registry.yaml` as a single YAML file.
...
categories:
  - id: "tasks"
    ...
    basePath: ".hefaisto-core/development/tasks"
```
...
```js
const { RegistryLoader } = require('./.hefaisto-core/core/ids/registry-loader');
...
```bash
node .hefaisto-core/development/scripts/populate-entity-registry.js
```
