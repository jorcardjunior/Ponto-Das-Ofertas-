 ...
 *
 * Layer Model:
 *   L1 (Framework Core)      — .hefaisto-core/core/, bin/, constitution.md
 *   L2 (Framework Templates)  — .hefaisto-core/development/, infrastructure/, product/
 *   L3 (Project Config)       — .hefaisto-core/data/, MEMORY.md, .claude/, *-config.yaml
...
const LAYER_RULES = [
  // --- L1: Framework Core ---
  { layer: 'L1', test: (p) => p.startsWith('.hefaisto-core/core/') },
  { layer: 'L1', test: (p) => p.startsWith('bin/') },
  { layer: 'L1', test: (p) => p === '.hefaisto-core/constitution.md' },

  // --- L3: Project Config (before L2 to catch MEMORY.md inside agents/) ---
  { layer: 'L3', test: (p) => p.startsWith('.hefaisto-core/data/') },
  ...
  { layer: 'L2', test: (p) => p.startsWith('.hefaisto-core/development/') },
  { layer: 'L2', test: (p) => p.startsWith('.hefaisto-core/infrastructure/') },
  { layer: 'L2', test: (p) => p.startsWith('.hefaisto-core/product/') },
