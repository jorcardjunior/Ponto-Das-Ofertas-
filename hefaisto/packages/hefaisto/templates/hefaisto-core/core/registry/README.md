# Hefaisto Service Registry

The Service Registry is the central catalog of all workers, tasks, templates, scripts, and workflows in the Hefaisto framework. It enables service discovery, search, and reuse across the system.
...
```javascript
const { getRegistry, loadRegistry } = require('.hefaisto-core/core/registry/registry-loader');
...
```bash
node .hefaisto-core/core/registry/build-registry.js
```

The builder scans:
- `.hefaisto-core/development/tasks/**/*.md` → task
- `.hefaisto-core/product/templates/**/*.md` → template
- `.hefaisto-core/infrastructure/scripts/**/*.js` → script
- `.hefaisto-core/product/checklists/**/*.md` → checklist
- `.hefaisto-core/development/workflows/**/*.yaml` → workflow
- `.hefaisto-core/core/data/**/*` → data
...
```bash
node .hefaisto-core/core/registry/validate-registry.js
```
...
```json
{
  ...
  "path": ".hefaisto-core/development/tasks/po-create-story.md",
...
- **Discovery CLI** (Story 2.7-2.9): `hefaisto search`, `hefaisto info`, `hefaisto list`
...
*Generated by Hefaisto Framework v4.0.4*
