 ...
 * Pro Command Module
 *
 * CLI commands for Hefaisto Pro license management and feature gating.
 *
 * Subcommands:
 *   hefaisto pro activate --key <KEY>    Activate a license key
 *   hefaisto pro status                   Show license status
 *   hefaisto pro deactivate               Deactivate the current license
 *   hefaisto pro features                 List all pro features
 *   hefaisto pro validate                 Force online revalidation
 *   hefaisto pro setup                    Configure GitHub Packages access (AC-12)
 *
...
// In framework-dev: __dirname = hefaisto-core/.hefaisto-core/cli/commands/pro → ../../../../pro/license
// In project-dev: pro is installed via npm as @hefaisto-fullstack/pro
function resolveLicensePath() {
  ...
  // 2. Try node_modules/@hefaisto-fullstack/pro/license (project-dev mode)
  try {
    const proPkg = require.resolve('@hefaisto-fullstack/pro/package.json');
    ...
    // @hefaisto-fullstack/pro not installed via npm
  }

  // 3. Try project root node_modules (fallback)
  const projectRoot = process.cwd();
  const cwdPath = path.join(projectRoot, 'node_modules', '@hefaisto-fullstack', 'pro', 'license');
...
function loadLicenseModules() {
  try {
    ...
    console.error('Hefaisto Pro license module not available.');
    console.error('Install Hefaisto Pro: npm install @hefaisto-fullstack/pro');
    process.exit(1);
  }
}

/**
 * Get Hefaisto Core version from package.json
 */
function getHefaistoCoreVersion() {
  try {
...
async function activateAction(options) {
  ...
  if (!key) {
    ...
    console.error('Usage: hefaisto pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
    process.exit(1);
  }
  ...
  console.log('\nActivating Hefaisto Pro license...');
  console.log(`Key: ${maskKey(key)}`);
  console.log('');

  try {
    const machineId = generateMachineId();
    const hefaistoCoreVersion = getHefaistoCoreVersion();

    const result = await licenseApi.activate(key, machineId, hefaistoCoreVersion);
    ...
    const proSourceDir = path.join(projectRoot, 'node_modules', '@hefaisto-fullstack', 'pro');
    ...
    if (fs.existsSync(proSourceDir)) {
      ...
      if (scaffoldProContent) {
        ...
        if (scaffoldResult.success) {
          ...
          console.error('Try running "hefaisto pro activate" again to retry scaffolding.');
        }
        console.log('');
      }
    } else {
      console.log('Note: @hefaisto-fullstack/pro package not found in node_modules.');
...
function statusAction() {
  ...
  if (!cache) {
    ...
    console.log('  Activate: hefaisto pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  ...
  if (info && info.inGrace) {
    ...
    console.log('  Please revalidate your license: hefaisto pro validate');
  }
...
async function deactivateAction(options) {
  ...
  if (!options.force) {
    console.log('\nDeactivating Hefaisto Pro License');
    console.log(`Key: ${maskKey(cache.key)}`);
    console.log('\nThis will:');
  ...
  try {
    ...
    console.log('');
    console.log('To reactivate: hefaisto pro activate --key <KEY>');
    console.log('');
...
async function validateAction() {
  ...
  console.log('\nValidating Hefaisto Pro license...\n');
...
// ---------------------------------------------------------------------------
// hefaisto pro setup (AC-12: Install-gate)
// ---------------------------------------------------------------------------

/**
 * Setup and verify @hefaisto-fullstack/pro installation.
 *
 * Since @hefaisto-fullstack/pro is published on the public npm registry,
...
async function setupAction(options) {
  ...
  if (options.verify) {
    // Verify-only mode
    console.log('Verifying @hefaisto-fullstack/pro installation...\n');

    try {
      const { execSync } = require('child_process');
      const result = execSync('npm ls @hefaisto-fullstack/pro --json', {
        stdio: 'pipe',
        timeout: 15000,
      });
      const parsed = JSON.parse(result.toString());
      const deps = parsed.dependencies || {};
      if (deps['@hefaisto-fullstack/pro']) {
        console.log(`✅ @hefaisto-fullstack/pro@${deps['@hefaisto-fullstack/pro'].version} is installed`);
      } else {
        console.log('❌ @hefaisto-fullstack/pro is not installed');
        console.log('');
        console.log('Install with:');
        console.log('  npm install @hefaisto-fullstack/pro');
      }
    } catch {
      console.log('❌ @hefaisto-fullstack/pro is not installed');
      console.log('');
  ...
  // Install mode
  console.log('@hefaisto-fullstack/pro is available on the public npm registry.');
  console.log('No special tokens or configuration needed.\n');

  console.log('Installing @hefaisto-fullstack/pro...\n');

  try {
    const { execSync } = require('child_process');
    execSync('npm install @hefaisto-fullstack/pro', {
      stdio: 'inherit',
      timeout: 120000,
    });
    console.log('\n✅ @hefaisto-fullstack/pro installed successfully!');
  } catch (error) {
  ...
  console.log('  hefaisto pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('');
  console.log('To check license status:');
  console.log('  hefaisto pro status');
  console.log('');
...
/**
 * Create the `hefaisto pro` command with all subcommands.
 * @returns {Command}
 */
function createProCommand() {
  const proCmd = new Command('pro')
    .description('Hefaisto Pro license management');
  ...
  proCmd
    .command('setup')
    .description('Install and verify @hefaisto-fullstack/pro')
