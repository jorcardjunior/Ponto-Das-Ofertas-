 ...
 */
async function validateStructure(hefaistoCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  const rootEntries = await fs.promises.readdir(hefaistoCoreDir, { withFileTypes: true });
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(hefaistoCoreDir);
  ...
  result.imports = await verifyImports(hefaistoCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push('  3. Report issues: github.com/hefaisto/issues');
  lines.push('');
  lines.push('To rollback: hefaisto migrate --rollback');
