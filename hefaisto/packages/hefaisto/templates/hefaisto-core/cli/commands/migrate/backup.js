...
function createBackupDirName() {
  ...
  return `.hefaisto-backup-${dateStr}`;
}
...
/**
 * Create backup of .hefaisto-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  // Check if .hefaisto-core exists
  if (!fs.existsSync(hefaistoCoreDir)) {
    throw new Error('No .hefaisto-core directory found. Is this an Hefaisto v2.0 project?');
  }
  ...
  const files = await getAllFiles(hefaistoCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(hefaistoCoreDir, srcFile);
    const destFile = path.join(backupDir, '.hefaisto-core', relativePath);
  ...
  const configFiles = [
    'hefaisto.config.js',
    'hefaisto.config.json',
    '.hefaisto/config.yaml',
    '.mcp.json',
  ];
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid Hefaisto backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.hefaisto-core', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.hefaisto-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.hefaisto-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.hefaisto-backup-')) {
