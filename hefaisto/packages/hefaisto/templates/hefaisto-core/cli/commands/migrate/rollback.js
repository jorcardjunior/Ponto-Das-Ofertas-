 ...
 */
async function removeV21Structure(hefaistoCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupHefaistoCoreDir = path.join(backup.path, '.hefaisto-core');

  // Restore .hefaisto-core files
  ...
  for (const file of backup.manifest.files) {
    try {
      const sourcePath = file.isConfig
        ...
        : path.join(backupHefaistoCoreDir, file.relativePath);

      const targetPath = file.isConfig
        ? path.join(projectRoot, file.relativePath)
        : path.join(hefaistoCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const hefaistoCoreDir = path.join(projectRoot, '.hefaisto-core');
  result.removal = await removeV21Structure(hefaistoCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(hefaistoCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push('');
  lines.push('🔙 Hefaisto Migration Rollback');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
