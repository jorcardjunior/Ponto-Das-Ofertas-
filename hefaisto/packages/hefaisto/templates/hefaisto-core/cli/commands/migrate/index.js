...
function printHeader(fromVersion, toVersion) {
  console.log('');
  console.log(`🔄 Hefaisto Migration v${fromVersion} → v${toVersion}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || 'Cannot detect Hefaisto version'}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log('  Run: hefaisto migrate --rollback');
      process.exit(1);
    }
  ...
  try {
    ...
    const importResult = await updateAllImports(hefaistoCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.hefaisto-backup-${new Date().toISOString().split('T')[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log('');
  console.log('🔙 Hefaisto Migration Rollback');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
...
async function showBackupStatus(_options) {
  ...
  console.log('');
  console.log('📦 Hefaisto Migration Backups');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function createMigrateCommand() {
  ...
  migrate
    .description('Migrate Hefaisto from v2.0 to v4.0.4 modular structure')
...
Migration Flow:
  Phase 1: Backup
    - Creates .hefaisto-backup-{date}/ with all files
...
Examples:
  $ hefaisto migrate                          # Interactive migration
  $ hefaisto migrate --dry-run                # Preview changes
  $ hefaisto migrate --verbose                # Detailed output
  $ hefaisto migrate -y --skip-tests          # Non-interactive, skip tests
  $ hefaisto migrate --rollback               # Restore from backup
  $ hefaisto migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ hefaisto migrate --rollback

  Manual rollback:
  $ cp -r .hefaisto-backup-{date}/* .hefaisto-core/
`);
