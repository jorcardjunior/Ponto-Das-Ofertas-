...
Hefaisto Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum Hefaisto setup | dev                | Quick prototyping |
...
\`\`\`bash
hefaisto install --preset my-preset
\`\`\`
```
...
hefaisto install --preset standard

# Install with multiple presets (layered)
hefaisto install --preset standard --preset frontend

# Install custom preset from directory
hefaisto install --preset ./presets/my-preset
```
...
hefaisto presets list

# List with details
hefaisto presets list --verbose
```
...
hefaisto presets show standard

# Compare two presets
hefaisto presets diff minimal standard
```
...
hefaisto presets export --name my-project-preset --output ./presets/

# Export with description
hefaisto presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
hefaisto install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `hefaisto presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading Hefaisto
...
_Last Updated: 2026-01-28 | Hefaisto Framework Team_
