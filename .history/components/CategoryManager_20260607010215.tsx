    ...
    <div className="mt-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="flex items-center gap-2">
          {PALETTE.map((c) => (
            <button key={c} type="button" onClick={() => setColor(c)} className={`h-8 w-8 rounded-full ${getCategoryColorClass(c)} ring-2 ${color === c ? 'ring-offset-2 ring-sky-400' : ''}`} aria-label={c} />
          ))}
        </div>
      </div>
        ...
        {categories.length === 0 ? (
          ...
          categories.map((c) => (
            <div key={c.name} className="inline-flex items-center gap-2 rounded-full bg-slate-50 px-3 py-1 text-sm">
              <span className={`h-3 w-3 rounded-full ${getCategoryColorClass(c.color)}`} />
