    ...
    <div className="mt-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="flex items-center gap-2">
          {PALETTE.map((c) => (
            <button key={c} type="button" onClick={() => setColor(c)} className={`h-8 w-8 rounded-full ring-2 ${color === c ? 'ring-offset-2 ring-sky-400' : ''}`} style={{ background: c }} aria-label={c} />
          ))}
        </div>
      </div>
      ...
      <div className="mt-3 flex gap-3">
        ...
        {onDelete && (
          <button onClick={() => onDelete(name)} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm">Remover</button>
        )}
      </div>
