...
const marketplaceStyles: Record<string, string> = {
  ...
  'Mercado Livre': 'bg-emerald-100 text-emerald-800',
  Amazon: 'bg-yellow-100 text-amber-800',
};

export function ProductTable({ products, categories = [], lowStockThreshold, onEdit, onDelete }: ProductTableProps) {
  return (
    <div className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-xl shadow-slate-200/40">
      <div className="px-6 py-5 border-b border-slate-200 bg-slate-50">
        ...
        <p className="mt-1 text-sm text-slate-500">Gerencie itens, categorias e estoque em um único painel.</p>
      </div>
          ...
          <tbody>
            {products.length === 0 ? (
              ...
              products.map((product) => {
                ...
                return (
                  <tr key={product.id} className="border-b border-slate-200 last:border-b-0 transition hover:bg-slate-50">
                    <td className="px-5 py-4">
                      <div className="font-semibold text-slate-900">{product.name}</div>
                      <div className="mt-1 text-xs text-slate-500">SKU: {product.sku}</div>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex rounded-2xl px-3 py-1 text-xs font-semibold ${marketplaceStyles[product.marketplace] ?? 'bg-slate-100 text-slate-700'}`}>
                        {product.marketplace}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-slate-700">
                      <div className="inline-flex items-center gap-2">
                        <span
                          className="h-3 w-3 rounded-full bg-[var(--category-color)]"
                          style={{ '--category-color': (categories.find((c) => c.name === product.category)?.color) ?? '#94a3b8' } as React.CSSProperties}
                        />
