    ...
    <form onSubmit={handleSubmit} className="space-y-6 rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_30px_100px_-60px_rgba(15,23,42,0.2)]">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {categories.map((c) => (
              <button key={c.name} type="button" onClick={() => setCategory(c.name)} className="inline-flex items-center gap-2 rounded-full bg-slate-50 px-3 py-1 text-sm" style={{ '--category-color': c.color } as any}>
