...
visual_methodology:
  ai_image_prompts:
    ...
    primary_engine: "OpenAI gpt-image-2 (auth via OPENAI_API_KEY)"
...
execution:
  ...
  default_params:
    ...
    n: 1
    response_format: b64_json  # gpt-image-2 retorna b64 por padrão
  output_location:
  ...
  bash_pattern_bash: |
    ...
    curl https://api.openai.com/v1/images/generations \
      ...
      -d "{\"model\":\"gpt-image-2\",\"prompt\":\"$PROMPT\",\"size\":\"1024x1024\",\"quality\":\"high\",\"n\":1}" \
  ...
  cost_awareness:
    - "gpt-image-2 cobra por imagem + por tokens do prompt — confirmar com usuário antes de lotes >5 imagens"
...
5. **Generate via OpenAI.** Chamar `POST /v1/images/generations` com `gpt-image-2` (ver bloco `execution` no YAML acima).
...
The Visual Generator turns brand strategy into visual reality — one precisely crafted asset at a time, **via OpenAI gpt-image-2**.
